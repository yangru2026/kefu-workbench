-- 修正版：去掉重复的 ALTER PUBLICATION，只执行剩余部分
-- 如果表已存在会自动跳过
...
DROP POLICY IF EXISTS "所有人可查看排名" ON ranking_data;
CREATE POLICY "所有人可查看排名" ON ranking_data FOR SELECT USING (true);
DROP POLICY IF EXISTS "管理员可管理排名" ON ranking_data;
...
DROP POLICY IF EXISTS "所有人可查看售前数据" ON presale_data;
CREATE POLICY "所有人可查看售前数据" ON presale_data FOR SELECT USING (true);
DROP POLICY IF EXISTS "管理员可管理售前数据" ON presale_data;
