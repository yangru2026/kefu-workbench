-- ============================================================
-- 售前话术：新增「操作类」话题 + 3 条示例场景（建模版）
-- 独立追加文件，单独在 Supabase SQL Editor 执行即可，不影响已有的 83 条话术。
-- 幂等：重复执行不会重复插入，也不会删除任何已有数据。
-- 注意：本文件只做「新增」。请勿再执行旧的全量文件 add_scripts_from_excel.sql，
--       因为那个文件第 59 行有 delete 全部售前话术的逻辑，重跑会清掉本文件加的内容。
-- ============================================================
...
-- 1) 新增「操作类」小类（parent 指向售前话术大类），sort_order 580 排在最后
...
-- 2) 插入 3 条示例场景话术（操作类）。全部归「通用」分组，方便你先看到模板。
--    下面 styles 仅「标准」风格填了建模示例，其余留空，你可在界面点卡片编辑补充。
...
-- 完成提示
select '操作类话题 + 3 条示例场景已就绪' as result;
